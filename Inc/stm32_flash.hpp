...
//TODO enable HSI!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	void [[gnu::always_inline]] WaitForLastOperation(){
		...
		if((FLASH->SR & FLASH_FLAG_BSY) != FLASH_COMPLETE){
	...
	void [[gnu::always_inline]] erasePage(const uint32_t page_num){
		...
		FLASH->AR  = static_cast<uint16_t>(flash_addr + (pageSize * page_num));
	...
	void [[gnu::always_inline]] reprogramPage(const std::array<uint8_t, pageSize>& Buff, const uint32_t page_num){
		unlock_flash asd;
